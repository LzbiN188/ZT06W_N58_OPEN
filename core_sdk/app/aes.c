...
//	for(int i = 0; i < 4; i++){
//		for(int j = 0; j < 4; j++)
//			LogPrintf("a[%d][%d] = 0x%x ", i, j, a[i][j]);
//		LogPrintf("\n");
//	}
//	LogPrintf("\n");
//}
...
//	for(int i = 0; i < len; i++)
//		LogPrintf("0x%x ", getIntFromChar(str[i]));
//	LogPrintf("\n");
//}
...
void aes(char *p, int plen, char *key)
{
    ...
    if (plen == 0 || plen % 16 != 0)
    {
        //		LogPrintf("明文字符长度必须为16的倍数！\n");
        return;
    }

    if (!checkKeyLen(keylen))
    {
        //		LogPrintf("密钥字符长度错误！长度必须为16、24和32。当前长度为%d\n",keylen);
        return;
    }
...
char deAes(char *c, int clen, char *key)
{
    ...
    if (clen == 0 || clen % 16 != 0)
    {
        //		LogPrintf("密文字符长度必须为16的倍数！现在的长度为%d\n",clen);
        return 0;
    }
