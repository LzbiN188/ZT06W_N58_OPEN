...
static gpsRestore_s gpsRestoreBuff[RESTORE_MAX_SZIE];
static batch_upload_s bus;
...
void jt808RegisterLoginInfo(uint8_t *sn, uint8_t reg, uint8_t *authCode, uint8_t authLen)
{
    ...
    changeByteArrayToHexString(jt808_info.jt808sn, (uint8_t *)debugData, 6);
    debugData[12] = 0;
...
static uint16_t jt808Escape(uint8_t *dest, uint16_t len)
{
    ...
    changeByteArrayToHexString(dest, senddata, debugLen);
...
{
    bus.type = type;
    if (bus.num < RESTORE_MAX_SZIE)
    {
        memcpy(&bus.grs[bus.num], grs, sizeof(gpsRestore_s));
        bus.num++;
        LogPrintf(DEBUG_ALL, "batch pushin:%d", bus.num);
    }
    if (bus.num >= RESTORE_MAX_SZIE)
    {
...
void jt808BatchPushIn(gpsinfo_s *gpsinfo)
{
    ...
    if (gpsinfo->hadupload || gpsinfo->fixstatus == 0)
    {
        return;
    }
    jt808PackGpsinfo(gpsinfo);
    jt808gpsRestoreSave(gpsinfo);
    gpsinfo->hadupload = 1;
    jt808BatchRestorePushIn(&jt808_gpsres, 0);
}
...
void jt808BatchPushOut(void)
{
    ...
    if (bus.num == 0 || bus.grs == NULL)
    {
        return;
    }
    LogPrintf(DEBUG_ALL, "batch pushout,total:%d", bus.num);
    if (serverIsReady())
    {
        jt808SendToServer(TERMINAL_BATCHUPLOAD, &bus);
    }
    else
    {
        gpsRestoreWriteData(bus.grs, bus.num);
    }
    bus.num = 0;
}
